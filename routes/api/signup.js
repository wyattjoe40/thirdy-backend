...
router.post('/', (req, res) => {
  ...
  function addToDatabase() {
    ...
    newUser.save((err, userSaved) => {
      ...
      if (userSaved.newsletterOptIn) {
        newsletter.api.addUser(userSaved.email).then(() => {
          console.log("Added email to newsletter: " + userSaved.email)
        }).catch((err) => {
          console.log("Error subscribing the email to newsletter: " + userSaved.email)
          console.log(err)
        })
      }
